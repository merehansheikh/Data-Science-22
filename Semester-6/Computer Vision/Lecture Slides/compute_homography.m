...
% Loop through the first point till the 4th point or the last col
for iPoint = 1:size(X,2)                          
    x = X(:,iPoint);                              % First point of the left image
    xprime = Xprime(:,iPoint);                    % First point of the right image

    % The equation is
    % xprime(1) = ( h11 x(1) + h12 x(2) + h13 ) / ( h31 x(1) + h32 x(2) + h33 )
    % Put this in the form ( coeffs * (h11 h12 h13 h21 h22 h23 h31 h32 h33)' ):
	% -x1 -y1 -1, 0 0 0, x1*x1prime x1prime*y1 x1prime. Here x1prime is xprime(1) and y1 is x(2)
    ...
    % 0 0 0, -x1 -y1 -1, x1 y1prime y1prime. Here y1prime is xprime(2) and y1 is x(2)
...
hold on;
plot(X(1,:),X(2,:),'bo');
axis([0,640,0,480],'equal','ij');
title('Left image points X');
hold off;

% Transfer the test points
% repmat will repeat copies of array
